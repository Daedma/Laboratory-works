...
// Параллельный алгоритм с использованием операций "точка-точка"
		...
		if (proc_rank == 0)
		{
			...
			{
				MPI_Recv(&proc_sum, 1, MPI_DOUBLE, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				total_sum += proc_sum;
			}
		}
		else
	...
	if (proc_rank == 0)
	{
		...
		printf("Parallel time (point-to-point): %f\n", tpp);
		printf("Parallel time (reduce): %f\n", tR);
		printf("Speedup (point-to-point with broadcast): %f\n", aRB);
		printf("Speedup (reduce with broadcast): %f\n", appB);
		printf("Speedup (point-to-point without broadcast): %f\n", aR);
		printf("Speedup (reduce without broadcast): %f\n", ap);
	}
