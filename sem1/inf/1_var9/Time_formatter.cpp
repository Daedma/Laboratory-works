...
начала суток, в формат «чсы:минуты:секунды». 
Входные  данные. Целое  неотрицатльное  число –  количество 
секунд, прошедших начала суток. 
Выходные данные. Количество часов, прошедших с начала су-
ток, количество минут, прошедших с начала последнего часа, и ко-
личество секунд, прошедших с начал последней минуты, в формате 
«часы:минуты:секунды».
*/
#include <iostream>
#include <sstream>
...
public:
    ...
    std::string format() const; //метод для приведения времени к отформатированной строке
...
void Time::add_seconds(uint64_t sec) noexcept
{
    ...
    Hour_ = (sec / 3600ULL) % 24ULL; //в скобках получаем количество часов и оставляем то, что умещается в сутки
}
...
int main()
{
    ...
    do
    {
        ...
        uint64_t seconds { 0 };
        while (std::cin.peek() == '-' || !(std::cin >> seconds) || std::cin.peek() != '\n') //пока ввод не будет успешным
        {
            std::cin.clear();                                                   //сбросим флаги ошибок
            if (std::cin.peek() == '-' || seconds == std::numeric_limits<uint64_t>::max())  //если число отрицательное или слишком большое, то ...
                std::cout << "The number must belong to the segment [0; "
                << std::numeric_limits<uint64_t>::max() << "]. Please try again>"; //... сообщить об этом пользователю
            else
        ...
        std::cout << time << std::endl; //выведем результат
